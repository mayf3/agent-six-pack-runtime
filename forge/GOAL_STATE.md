...
### GOAL DAY_DSH_SCOUT_QUEUE_RECONCILIATION_20260920_V1（Owner 注入；OWNER_QUEUE_RECONCILIATION 履行完毕 @ 本 push）

- **§1 fresh truth**：dsh main=**a5fed401**（与 GOAL 参考一致）；runtime origin head=**64c062f**；09-19/20 多轮 DSH Scout 报告对账对象全部 fresh 读取。
- **§2 T42/T67 MERGED 后 relevance reopen**：fresh a5fed401 实证残差——T42：trackedTick()（engine.js L483-487）每 tick 无条件覆写 inflight（busy/skipped tick 冲掉活跃 poll 的 drain 引用）、start() L510 `void reconcileOnce()` 启动对账在 drain 归属外、wrapper 无 stop/drain 转发；T67：entry.js L92-100 stop 失败被 catch 后**仍无条件**写 stopped 证据+cleanly+exit(0)。两票追加 POST_MERGE_RESIDUAL_DISCOVERED 块（PREVIOUS_REPAIR=dsh#282 / CURRENT_RELEVANCE=STILL_ACTIONABLE / ROOT_CAUSE_GROUP=DSH_SHUTDOWN_CONTRACT）+severity P1+经 set-ticket-state 前向迁移 **MERGED/WAITING_OWNER_DECISION → READY_FOR_REVALIDATION**（reopen 双/单通过，read-back OK）。原 MERGED 历史（T42）与先前尾部（T67）保留不删。workstream 维持双面：T42=drain 边界、T67=result/exit truth。
- **§3 唯一新 finding materialize=T92 = WORKFLOW-RECOVERING-QUIESCENCE-V1**（P1，READY_FOR_REPRODUCTION 只读表征准入）：DSH-SCOUT-03/C01/SCOUT-20260919-WORKFLOW-RECOVERING-GATE/SCOUT-A5F-WFE-QUIESCENCE 四 alias 去重统一；fresh 实证=Router recovering（query.js:47）不被 re-entry 延迟门覆盖（engine.js:152-157 仅查 pending）、admitDueIntent 可达 beginAttemptIfAbsent（V1 L198 唯一 mint）；frozen acceptance（无正证明→无 N+1/无 attempt/无 delivery）+四禁项落档；accepted contracts 已定义语义→OWNER_DECISION_REQUIRED=NO；**写准入 OWNER_BLOCKED_FOR_WRITE（本 GOAL 未授 mandate word，不自动实现）**。
- **§4 #265→#268 stacked source closure=T93 = DSH-PROV-STACKED-SOURCE-CLOSURE-01**（P2，READY_FOR_REVALIDATION）：fresh 实证 main 全树无 storeWriteOwner（#265 OPEN/CLEAN f79797c 仍 needed）、client 选择仍 row-id 优先（#268 OPEN/CLEAN df5a60a stacked 于 #265 分支）；STACKED_RELATION=PARENT→#268 CHILD 不因 stacked 关闭；Nightly 合法工作=当前 main integration+bounded regression+exact-head 增量评审+source closure（明确不新建实现票/不得第三张 provisioning repair）。
- **§5 NOT_MATERIALIZED（follow-up/validate-first，零新票）**：#196 install revalidation、authority index drift、stale PR disposition、History mount/dispose race、Scheduler admission attribution、offline-pnpm ordering、AgentProcess cleanup boundary、scoped-successor governance-form conflict。§6 resolved truth 保持：T48 族 authority=accepted/implementation=main/deployed（09-15 验证）/consumer 单独 gate——零重开。
- **§8 机械可见性**：lint PASS **88 票/0 findings**；plan EXECUTE_PLAN order=4（T42/T67/T92/T93 全 EXECUTE_READONLY）；roundrobin EXECUTE_QUEUE_ITEM×4 同序；wake recomputed=6/true_idle=False/WAKE_RESUME；drive EXECUTE_QUEUE_ITEM/legal_terminal=None。**NIGHTLY_VISIBLE_WORKSTREAMS=3/3**（P1-1 shutdown closure 双面、P1-2 recovering、P2-1 stacked closure）。NEW_DUPLICATE_TICKETS=0。修复两处自陷：T92 canonical 名下划线不符 head 正则→改连字符；新块小写 closure 子串命中大小写不敏感标记表→改「收口」。
- PRODUCT_CODE_MUTATIONS=0 / PR_CODE_MUTATIONS=0 / MERGES=0 / DEPLOYS=0 / PRODUCTION_WRITES=0。receipt=state/dispatch/2026-09-20/receipts/dsh-scout-reconciliation.json。账 @ 本 push。

### 09-23 夜 BOOTSTRAP + 四票履约终态（NIGHT_RUN_ID=2026-09-23-nightly-dispatch-v1）

- 23:00 BOOTSTRAP：gate 6/6、executable=4（T42/T67 重开残差 + T92/T93 新票，日班 DAY_DSH_SCOUT_QUEUE_RECONCILIATION_20260920_V1 产物）→ 取得执行权。09-20~09-23 各窗无触发直接跳本夜。
- **T42+T67 r2（dsh#317 Draft @71575eb）**：shutdown contract 三残差面收口——①trackedTick busy-tick 不再覆写 drain 归属（busy 返回活跃 drain promise；pre-r2 noop promise 即时结算清槽→stop() 立即返回真 poll 失管）；②catchup reconcile 入 drain（stop() 双等待）；③entry 真实结果面（shutdownResultFor/applyShutdownResult 零依赖模块：stop 失败=>stop_failed+exit1，pre-r2 吞错仍 cleanly/exit0）。**评审三轮课程**：round2 ACCEPT 后评审员自查发现 entry binding blocker（间接 re-export 无本地绑定→ReferenceError 机械复现）REJECT、round3 REJECT 同类未绑定 writeEvidence（stub 掩盖、stub 驱动真 entry 复现）→ round4 收敛单行修复（writeEvidence: runtime.writeEvidence）ACCEPT 0 blockers。回归：r2 leg A/B RED→GREEN + r1 腿不变 + shutdown-result 3/3，共 7/7。wrapper 表征记档：thin launcher 同进程、信号直达（scout"无转发路径"系误读）。
